...
public class Commands implements CommandExecutor, TabCompleter {
    ...
    private static final List<String> SUBCOMMANDS = Arrays.asList("help", "set", "delete", "list", "reload");
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            // Подсказываем подкоманды с учётом прав
            String input = args[0].toLowerCase();
            for (String sub : SUBCOMMANDS) {
                if (sub.startsWith(input) && sender.hasPermission("worldborder." + sub)) {
                    suggestions.add(sub);
                }
            }
        } else if (args.length == 2) {
            String sub = args[0].toLowerCase();
            // Для set и delete — список миров
